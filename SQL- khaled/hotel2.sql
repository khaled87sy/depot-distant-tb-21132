...
SELECT cli_nom, hot_nom, res_date_debut, (SELECT DATEDIFF(res_date_fin, res_date_debut)) AS "Duree_Sejour" FROM reservation
JOIN client ON res_cli_id = cli_id
JOIN chambre ON res_cha_id = cha_id
JOIN hotel ON cha_hot_id = hot_id
...
SELECT sta_nom AS "NOM d'HOTEL", COUNT(hot_nom) AS "Le nombre d’hôtel par station" FROM hotel, station 
WHERE sta_id = hot_sta_id 
GROUP BY sta_nom

-- 14 - Compter le nombre de chambre par station
SELECT sta_nom AS "NOM DE LA STATION", COUNT(cha_hot_id) AS "le nombre de chambre par station" FROM chambre, hotel, station 
WHERE cha_hot_id = hot_sta_id AND hot_sta_id = sta_id 
GROUP BY sta_nom

-- 15 - Compter le nombre de chambre par station ayant une capacité > 1
SELECT sta_nom AS "NOM DE LA STATION", COUNT(cha_hot_id) AS "le nombre de chambre par station ayant une capacité > 1"  FROM chambre, hotel, station 
WHERE cha_hot_id = hot_sta_id AND hot_sta_id = sta_id AND cha_capacite>1
GROUP BY sta_nom
...
SELECT  ROUND(AVG(DATEDIFF(res_date_fin, res_date_debut))) AS  'la durée moyenne des réservations par station' FROM reservation
