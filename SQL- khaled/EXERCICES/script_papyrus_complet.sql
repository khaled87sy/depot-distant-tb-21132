...
SELECT fournis.numfou AS "LES fournisseurs avec commandes passées." FROM fournis
...
SELECT vente.codart, fournis.numfou, nomfou, sum(stkphy*stkale) AS "total des stocks" FROM fournis
JOIN vente ON fournis.numfou = vente.numfou
...
ORDER BY sum(stkphy*stkale) desc

--18-- En fin d'année, sortir la liste des produits dont la quantité réellement commandée dépasse 90% de la quantité annuelleprévue.
