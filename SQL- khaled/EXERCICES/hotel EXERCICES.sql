...
SELECT sta_nom AS "NOM de station", COUNT(hot_nom) AS "Le nombre d’hôtel par station" FROM hotel
...
SELECT sta_nom AS "NOM DE LA STATION", COUNT(cha_hot_id) AS "le nombre de chambre par station ayant une capacité > 1"  FROM station
JOIN hotel ON hot_sta_id = sta_id
JOIN chambre ON cha_hot_id = hot_sta_id
WHERE cha_capacite>1
GROUP BY sta_nom
